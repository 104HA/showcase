...
var mBox = new Class({
	...
	options: {
		...
		'class': 'mBox',				// add by mllee: prefix of class name
		...
		widthLimit: {				// (added by mllee)
			min: -1,
			max: -1,
			offset: 0
		},
		height: 'auto',				// height of the content area
		heightLimit: {				// (added by mllee)
			min: -1,
			max: -1,
			offset: 0
		},
		zIndex: 8000,					// z-index of wrapper
		locate: document.body,		// 'inline' or dom element (added by mllee)
		...
		setContent: null,			// if the attached element has the attribute data-setContent it's value will be set as new content on open
	...
	fixOptions: function() {
		...
		if(!this.options.locate)
			this.options.locate = document.body;
	},
	...
	construct: function() {
		...
		this.wrapper = new Element('div', {
			id: this.id,
			'class': this.options['class'] + ' ' + (this.defaultTheme || 'Core') + (this.options.theme ? '-' + this.options.theme : '') + ' ' + (this.options.addClass.wrapper || ''),
			styles: {
		...
		if(typeOf(this.options.locate) == 'element') this.wrapper.inject(this.options.locate, 'bottom');		// modified by mllee
		...
		this.content = new Element('div', {
			...
			styles: {
				width: this._calWidthOrHeight(true),	// (modified by mllee)	
				height: this._calWidthOrHeight(false)	// (modified by mllee)
			}
	...
	_calWidthOrHeight: function(width){	// (added by mllee)
		var value = width ? this.options.width : this.options.height;
		if(typeOf(value) == 'number') return value;
		else if(typeOf(value) == 'string'){
			if(value.lastIndexOf('px') == value.length - 2) return isNaN(value.toInt()) ? 'auto' : value.toInt(); 
			else if(value.lastIndexOf('%') == value.length - 1){
				var length = (window.getSize()[width ? 'x' : 'y'] * value.toInt() / 100).toInt();
				var limit = this.options[width ? 'widthLimit' : 'heightLimit'];
				if(limit.min > 0 && length < limit.min) length = limit.min;
				if(limit.max > 0 && length > limit.max) length = limit.max;
				if(limit.offset != 0) length += limit.offset;
				return length;
			}
			else return 'auto';
		}
	...
	open: function(options) {
		...
		if(!this.isOpen && !this.block && (!this.options.setContent || 
				(this.options.setContent && (this.source && this.source.getProperty(this.options.setContent) != null)))) {	// modified by mllee (fix for IE8)
			...
			var open = function(complete) {
				...
				this.content.setStyles({
					width: this._calWidthOrHeight(true),
					height: this._calWidthOrHeight(false)
				});
	...
	close: function(options) {
		...
		if(this.isOpen && !this.block) {
			...
			var complete = function() {
				...
				this.ignoreDelayOnce = false;
				if(this.wrapper) this.wrapper.setStyle('display', 'none');
	...
	fixOperaPositioning: function(el) {
		if($(el) && !$(el).retrieve('OperaBugFixed') && el != window && Browser.name=='opera') {
			try {
	...
	setPosition: function(target, position, offset) {
		...
		if(pointer && !this.pointerElement) {
			...
			if(Browser.opera) {
				var tempContainer = new Element('div', {'class': this.options['class'] + ' ' + (this.defaultTheme || 'Core') + (this.options.theme ? '-' + this.options.theme : '')}).inject(document.body).grab(this.pointerElement);
		...
		if(this.drag)	// added by mllee
			this.drag.setOptions({
				limit: {
					x: [0, window.getSize().x - wrapperDimensions.width],
					y: [0, window.getSize().y - wrapperDimensions.height]
				}
			});
		return this;
	},
	...
	setContent: function(content, where) {
		if(content != null) {
			if($(content) || $$('.' + content).length > 0) {
				if($(content) && !where && this.options.locate == 'inline') this.wrapper.inject($(content), 'after');			// add by mllee
	...
	setTitle: function(content) {
		if(content != null && !this.titleContainer) {
			...
			if(this.options.draggable && window['Drag'] != null) {
				this.drag = new Drag(this.wrapper, {handle: this.titleContainer});
	...
	setFooter: function(content) {
		...
		if(content != null) this.setContent(content, 'footer');
		return this;
	},
	...
	attachEvents: function() {
		...
		this.resizeEvent = function(ev) {
			...
			if(w.lastIndexOf('%') == w.length - 1) this.content.setStyle('width', this._calWidthOrHeight(true) + 'px');
			var h = this.options.height + '';
			if(h.lastIndexOf('%') == h.length - 1) this.content.setStyle('height', this._calWidthOrHeight(false) + 'px');
			this.setPosition();
		}.bind(this);
	...
	addButtons: function(buttons) {
		...
		buttons.each(function(button, index) {
			new Element('button', {
				...
				events: {
					'click': (button.event || this.close).bind(this)
				}
			}).inject(this.footer);
		}.bind(this));
...
mBox.Notice = new Class({
	...
	options: {
		...
		reuse: false		// (added by mllee) true will not close and destroy notice automaticlly
	},
	...
	initialize: function(options) {
		...
		options.onSystemBoxReady = function() {
			...
			if(this.options.move && (this.position.x[1] == 'inside' || this.position.x[0] == 'center') && this.position.y[1] == 'inside' && (this.position.y[0] == 'top' || this.position.y[0] == 'bottom')) {
				...
				this.wrapper.setStyles({
					height: 0,
					width: wrapper_dimensions.totalWidth + 2,
					overflowY: 'hidden'
				});
				...
				if(this.options.reuse) this.options.delayClose = 0;
				else this.options.delayClose += this.options.moveDuration;
			}
		};
	...
	close: function(){
		if(this.options.reuse) this.wrapper.setStyle('height', 0);
		return this.parent();
	}
});
