...
(function(global, $){
    ...
    Element.Events.paste = {
    	...
    	base : (Browser.platform == 'ios' || Browser.name == 'opera' || (Browser.name == 'firefox' && Browser.version < 3))? 'input': 'paste',		// fixed by mllee (for mootools 1.5.1)
    ...
    Meio.Mask = new Class({
        ...
        onMask: function(e, func){
            ...
            o.isBksKey = (keyCode == 8 || (Browser.platform == 'ios' && e.code == 127));			// fixed by mllee (for mootools 1.5.1)
    ...
    Meio.Mask.extend({
        ...
        if (Browser.platform == 'ios'){			// fixed by mllee (for mootools 1.5.1)
...
Meio.Mask.Fixed = new Class({
    ...
    focus: function(e, o){
        ...
        this.focusTimeout = this.element.setCaretPosition.delay(5, this.element, [this.getLastValidIndex(elementValue)]);
        this.parent(e, o);
    },
    ...
    keypress: function(e, o){
        ...
        if(!o.isSelection){
            ...
            this.element.set('value', this.maskMold.join('')).setCaretPosition(newCarretPosition);
...
Meio.Mask.createMasks('fixed', {
	...
	'cep': {mask: '99999-999'},
	'dateTime': {mask: '9999/1m/3d 2h:59', placeholder: ' '},
	'date': {mask: '9999/1m/3d', placeholder: ' '},
	'year': {mask: '9999', placeholder: ' '},
	'month': {mask: '1m', placeholder: ' '},
	'day': {mask: '3d', placeholder: ' '},
	'yearMonth': {mask: '9999/1m', placeholder: ' '},
	'monthDay': {mask: '1m/3d', placeholder: ' '},
	'time': {mask: '2h:59', placeholder: ' '},
	'am': {mask: 'AM11:59', placeholder: ' '},
	'pm': {mask: 'PM11:59', placeholder: ' '},
	'hour': {mask: '2h', placeholder: ' '},
	'minute': {mask: '59', placeholder: ' '},
	'cc': {mask: '9999 9999 9999 9999'},
	'mac': {mask: 'HH:HH:HH:HH:HH:HH'},
	'cellPhone': {mask: '0999-999999', placeholder: ' '},
	'int1': {mask: '9', placeholder: ' '},
	'int2': {mask: '99', placeholder: ' '},
	'int3': {mask: '999', placeholder: ' '},
	'int4': {mask: '9999', placeholder: ' '},
	'int5': {mask: '99999', placeholder: ' '},
	'int6': {mask: '999999', placeholder: ' '},
	'int7': {mask: '9999999', placeholder: ' '},
	'int8': {mask: '99999999', placeholder: ' '},
	'int9': {mask: '999999999', placeholder: ' '}
});
...
Meio.Mask.Reverse = new Class({
    ...
    keypress: function(e, o){
        ...
        var befCount = elementValue.match(/,/g) != null ? elementValue.match(/,/g).length : 0;  
        ...
        elementValue = this.forceMask(elementValue, true);
 		var aftCount = elementValue.match(/,/g) != null ? elementValue.match(/,/g).length : 0;  
        ...
        //原始的 this.element.set('value', elementValue).setCaretPosition(elementValue.length);
        //不想讓下面寫成一行怕太長，因為千分位讓指標少一位須做調整 且須跳脫刪除鍵位移
        var resize=0;
        if(befCount<aftCount) resize=1;
        if(befCount>aftCount) resize=-1;
        
        this.element.set('value', elementValue).setCaretPosition(state.start + (o.isRemoveKey ? 0 : 1)+resize);
...
Meio.Mask.createMasks('reverse', {
	'integer': {precision: 0, maxLength: 18, thousands: ',', decimal: '.'},
	'decimal': {thousands: ',', decimal: '.'},
	'reais': {symbol: 'R$ '},
	'currency': {symbol: '$ ', thousands: ',', decimal: '.', precision: 0},
	'USD': {symbol: 'USD$ ', thousands: ',', decimal: '.', precision: 2},
	'NTD': {symbol: 'NTD$ ', thousands: ',', decimal: '.', precision: 0},
	'CNY': {symbol: '¥ ', thousands: ',', decimal: '.', precision: 1},
	'JPY': {symbol: '¥ ', thousands: ',', decimal: '.', precision: 0},
	'EUR': {symbol: '€ '}
});
